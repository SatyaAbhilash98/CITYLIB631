...
public class SearchResult extends JDialog {
	...
	public SearchResult(String searchKeyword, int searchType, String readerId) {
		...
		String[] columnNames = {"DOCID", "TITLE", "PDATE", "PUBLISHERID", "PUBNAME", "COPYNO", "BID", "POSITION", "LNAME", "LIBADDRESS"};
		ArrayList<ArrayList<Object>> searchResult = new ArrayList<ArrayList<Object>>();
		
		if (searchType == SEARCH_TYPE_ID) {
			searchResult = m.execQuery("SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`BID`, `POSITION`, `LNAME`, `LOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`BID`=`B`.`BID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `D`.`DOCID` = '" + searchKeyword + "';");
		} else if (searchType == SEARCH_TYPE_TITLE) {
			searchResult = m.execQuery("SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`BID`, `POSITION`, `LNAME`, `LOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`BID`=`B`.`BID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `D`.`TITLE` LIKE '%" + searchKeyword + "%';");
		} else if (searchType == SEARCH_TYPE_PUBLISHER) {
			searchResult = m.execQuery("SELECT `D`.`DOCID`, `TITLE`, `PDATE`, `D`.`PUBLISHERID`, `P`.`PUBNAME`, `COPYNO`, `C`.`BID`, `POSITION`, `LNAME`, `LOCATION`  FROM `DOCUMENT` D, `PUBLISHER` P, `COPY` C, `BRANCH` B WHERE `D`.`DOCID`=`C`.`DOCID` AND `C`.`BID`=`B`.`BID` AND `D`.`PUBLISHERID` = `P`.`PUBLISHERID` AND `P`.`PUBNAME` LIKE '%" + searchKeyword + "%';");
		}
		...
		btnReserve.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				reservedResult = m.execQuery("SELECT * FROM RESERVES R WHERE `R`.`DOCID` = '" + searchArray[rowIndex][0] + "' AND `R`.`COPYNO` = '" + searchArray[rowIndex][5] + "' AND `R`.`BID` = '" + searchArray[rowIndex][6] +"';");
				...
				if (reservedResult == null || reservedResult.size() <= 0) {
					...
					borrowResult = m.execQuery("SELECT * FROM BORROWS B WHERE `B`.`DOCID` = '" + searchArray[rowIndex][0] + "' AND `B`.`COPYNO` = '" + searchArray[rowIndex][5] + "' AND `B`.`BID` = '" + searchArray[rowIndex][6] +"' AND `B`.`RDTIME` IS NULL;");
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						...
						count1 = m.execQuery("SELECT COUNT(*) FROM RESERVES WHERE RID = '" + readerId + "';");
						count2 = m.execQuery("SELECT COUNT(*) FROM BORROWS WHERE RID = '" + readerId + "' AND RDTIME IS NULL;");
						Long count = (Long)(count1.get(0).get(0)) + (Long)(count2.get(0).get(0));
						
						
						// If less than 10. then can reserve.
						if (count < 10) {
							int insertedRows = m.execUpdate("INSERT INTO RESERVES(`RID`, `DOCID`, `COPYNO`, `BID`, `DTIME`) VALUES('" + readerId + "','" + searchArray[rowIndex][0] + "','" + searchArray[rowIndex][5] + "','" + searchArray[rowIndex][6] +"','" + date + "' );");
							if (insertedRows != 1) {
				...
				// If exist in RESERVES table, should be one tuple.
					...
					if (((Integer)reserveArr[0][1]).toString().equals(readerId)) {
						int deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DOCID` = '" + searchArray[rowIndex][0] + "' AND `COPYNO` = '" + searchArray[rowIndex][5] + "' AND `BID` = '" + searchArray[rowIndex][6] +"';");
						if (deletedRows != 1) {
						...
						if (cl.getTimeInMillis() - ts < 0) {		
							...
							if ((System.currentTimeMillis() - cl.getTimeInMillis()) > 0) {	
								...
								if ((ts - 86400000) - cl.getTimeInMillis() < 0) {
									...
									readerReserved = m.execQuery("SELECT * FROM RESERVES WHERE `RID` = '" + readerId + "';");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (reservedResult == null || reservedResult.size() <= 0) {
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						...
						if (count < 10) {
							int insertedRows = m.execUpdate("INSERT INTO BORROWS(`RID`, `DOCID`, `COPYNO`, `BID`, `BDTIME`, `RDTIME`) VALUES('" + readerId + "','" + searchArray[rowIndex][0] + "','" + searchArray[rowIndex][5] + "','" + searchArray[rowIndex][6] +"', NOW(), NULL);");
							if (insertedRows != 1) {
				...
				// If exist in RESERVES table, should be one tuple.
					...
					if (((Integer)reserveArr[0][1]).toString().equals(readerId)) {
						int deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DOCID` = '" + searchArray[rowIndex][0] + "' AND `COPYNO` = '" + searchArray[rowIndex][5] + "' AND `BID` = '" + searchArray[rowIndex][6] +"';");
						/*
						 * if (deletedRows != 1) { JOptionPane.showMessageDialog(null,
						 * "Failed to remove reservation"); return; }
						 */
